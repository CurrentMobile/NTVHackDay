  ...
  override func viewDidLoad() {
    ...
    func loadPhoto() {
        ...
        if photo != nil {
            ...
            Alamofire.request(.GET, photo!.imageURL!).validate().responseImage() {
                (_, _, image, error) in
    ...
    if photo != nil && photo!.imageURL != nil {
        ...
        let destination: (NSURL, NSHTTPURLResponse) -> (NSURL) = {
            ...
            if let directoryURL = NSFileManager.defaultManager().URLsForDirectory(.DocumentDirectory, inDomains: .UserDomainMask)[0] as? NSURL {
                return directoryURL.URLByAppendingPathComponent("\(self.photo!.id).\(response.suggestedFilename)")
            }
            
            return temporaryURL
        }
        
        let progressIndicatorView = UIProgressView(frame: CGRect(x: 0.0, y: 80.0, width: self.view.bounds.size.width, height: 10))
        ...
        Alamofire.download(.GET, imageURL, destination).progress {
            ...
            dispatch_async(dispatch_get_main_queue()) {
                ...
                let progressColor = UIColor(red: 1.0 - progress, green: progress, blue: 0.0, alpha: 1.0)
                
                progressIndicatorView.tintColor = progressColor
                
                progressIndicatorView.setProgress(Float(progress), animated: true)
